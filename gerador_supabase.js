...
function ProductCard({ product, onClick }) {
  ...
  const uniqueColors = Array.from(new Map(allColors.map(c => [c.hex, c])).values()).slice(0, 6);

  const nextImg = (e) => {
    e.stopPropagation();
    if (product.images?.length > 1) setImgIdx((prev) => (prev + 1) % product.images.length);
  };

  const prevImg = (e) => {
    e.stopPropagation();
    if (product.images?.length > 1) setImgIdx((prev) => (prev - 1 + product.images.length) % product.images.length);
  };
  ...
  return (
    <div 
      ...
      className="bg-white rounded-xl overflow-hidden shadow-sm cursor-pointer group hover:shadow-md transition-all"
    >
      <div className="aspect-[3/4] bg-gray-100 relative overflow-hidden">
        {product.images?.[imgIdx] ? (
           <img src={product.images[imgIdx]} className="w-full h-full object-cover transition-transform duration-500" />
        ) : (<div className="w-full h-full flex items-center justify-center text-gray-300"><ImageIcon/></div>)}
        
        <div className="absolute top-2 left-2 bg-white/90 px-2 py-1 text-[10px] font-bold uppercase tracking-wider z-10">{product.category}</div>

        {hover && product.images?.length > 1 && (
          <>
            <button onClick={prevImg} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/80 p-1 rounded-full hover:bg-white shadow-sm z-20 transition-opacity"><ChevronLeft size={16}/></button>
            <button onClick={nextImg} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/80 p-1 rounded-full hover:bg-white shadow-sm z-20 transition-opacity"><ChevronRight size={16}/></button>
      ...
      <div className="p-4 space-y-2">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            {product.variants?.[0] ? \`REF: \${product.variants[0].ref}\` : 'S/ REF'}
          </p>
        </div>
        ...
        {uniqueColors.length > 0 && (
          <div className="flex gap-1 items-center pt-1">
            {uniqueColors.map((c, i) => (
              <div key={i} className="w-3 h-3 rounded-full border border-gray-200" style={{backgroundColor: c.hex}} title={c.name} />
            ))}
            {allColors.length > 6 && <span className="text-[10px] text-gray-400">+{allColors.length - 6}</span>}
          </div>
        )}

        <div className="pt-2 mt-1 border-t border-gray-50">
           <p className="text-[11px] text-gray-500 truncate">{product.composition || 'Composição não informada'}</p>
        </div>
      </div>
    </div>
  );
...
function CatalogView({ products, categories, onSelect }) {
  ...
  const filtered = products.filter(p => {
    ...
    const matchesSearch = 
      ...
      p.variants?.some(v => v.ref.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 border-b pb-4">
        ...
        <div className="relative w-full md:w-64">
           <input 
             ...
             className="w-full border rounded-full py-2 pl-10 pr-4 text-sm outline-none focus:border-black bg-gray-50 focus:bg-white transition-colors"
             value={searchTerm}
...
function ProductDetailView({ product, onBack, isAdmin, onEdit }) {
  ...
  useEffect(() => { setImg(product.images?.[0]) }, [product]);
...
function ProductForm({ existingProduct, categories, onSave, onCancel }) {
  ...
  // Função para tornar a imagem principal (mover para index 0)
    ...
    <div className="max-w-4xl mx-auto bg-white p-6 md:p-8 rounded-xl shadow-lg border animate-in zoom-in-95">
      ...
      <div className="space-y-6">
        ...
        <div className="border p-5 rounded-xl">
           <h3 className="font-bold text-sm uppercase mb-4 flex items-center gap-2"><ImageIcon size={14}/> Galeria (A 1ª é a Capa)</h3>
           <div className="grid grid-cols-4 gap-3 mb-4">
             {form.images.map((img, i) => (
               <div key={i} className="relative aspect-[3/4] group">
                 <img src={img} className={\`w-full h-full object-cover rounded-lg border transition-all \${i === 0 ? 'border-black border-4' : ''}\`} />
                 ...
                 {/* Botão Capa / Badge */}
                 {i === 0 ? (
                    <div className="absolute bottom-0 left-0 right-0 bg-black/70 text-white text-[10px] font-bold text-center py-1">CAPA PRINCIPAL</div>
                 ) : (
                    <button type="button" onClick={() => setMainImage(i)} className="absolute top-1 left-1 bg-white text-black p-1 rounded-full shadow opacity-0 group-hover:opacity-100 hover:bg-yellow-100 transition-all flex gap-1 items-center px-2 text-[10px] font-bold"><Star size={10} fill="black"/> Virar Capa</button>
                 )}
               </div>
             ))}
           </div>
...
const sqlHelp = `-- RODE ESTE COMANDO NO "SQL EDITOR" DO SEU SUPABASE PARA CRIAR O BANCO --
...
-- IMPORTANTE: Depois de rodar isso, vá em "Authentication > Policies" 
-- e desative o RLS (Row Level Security) ou crie uma politica "Enable all" 
-- para que o site consiga salvar sem login complexo.
`;
...
function createFile(pathStr, content) {
  const dir = path.dirname(pathStr);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  fs.writeFileSync(pathStr, content);
  console.log('Criado: ' + pathStr);
}

console.log('Iniciando projeto com Supabase e Carrossel...');

createFile('package.json', packageJson);
createFile('vite.config.js', viteConfig);
createFile('index.html', indexHtml);
createFile('vercel.json', vercelJson);
createFile('.gitignore', gitIgnore);
createFile('INSTRUCOES_BANCO.txt', sqlHelp);
createFile('src/main.jsx', mainJsx);
createFile('src/index.css', indexCss);
createFile('src/App.jsx', appJsx);

console.log('\\n✅ Projeto Supabase atualizado!');
console.log('👉 Lembre-se de colocar suas chaves do Supabase no src/App.jsx');
console.log('👉 Rode: npm install e depois npm run dev');